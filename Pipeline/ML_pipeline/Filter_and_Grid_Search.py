...
from numpy import histogram_bin_edges, histogram
...
def stratified_split(data, target, list_of_vars_for_strat, sort_by_var, size_of_test, drop_technical, random_state):
...
def attributes_list_new(data, 
    ...
    if show_progress == True:
        iterable = tqdm_notebook(data[columns].iteritems(), desc='Обработано признаков',total=data_width)
    else:
...
def get_stats(data, columns, time, cut, N, dropna= 'min', category_list = None, encode = False, groupped_s = 'auto', group = True, n_jobs=2, verbose=20):
...
def receive_correlations(data, categorial_list = None):
    ...
    tm = datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")
    print ('Начало', 'time: ' , tm)

    corr_num = data1[num_list].corr()
    
    tm = datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")
    print ('Корреляционная матрица по числовым переменным посчитана', 'time: ' , tm)
    
    corr_cat = data1[cat_list].corr(method = 'spearman')

    tm = datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")
    print ('Корреляционная матрица по категориальным переменным посчитана', 'time: ' , tm)
...
def find_doubles_corr(data, colls, CORR, definition = None, lvl = 0.90, light_unstable = None):
...
def make_standard(data, columns, target, attribute_list, technical_list = None, categorial_list = None, label_encoder = None, 
                 string_list = None, mis_value = None, small_treshold = 10):
...
def data_preprocessing_train(data, target, technical_values, categorial_list = None, drop_technical = False,
                                yeo_johnson = False, attribute_list = None, var_col = None,
                       scale = 'mean', median = 'median',
                      high_outlier = None, low_outlier = None, check_percentile = 1, cols_outlier = None, cut_non_out_9999 = True):
...
def data_preprocessing_test(data, target, technical_values, categorial_list = None, drop_technical = False,
                            attribute_list = None, var_col = None,
                        median = None,
                      high_outlier = None, low_outlier = None, scale = None, 
                                yeo_johnson = None, cols_outlier = None):
...
def two_forests(data1, y1, data2, y2, param_dict, task = 'binary', use_metric = None, treshold_metric = True, 
    ...
    if callable(use_metric):
        if (treshold_metric == True and task == 'binary') or task == 'numeric':
            def metric_our(model, X_valid, y_valid, sample_weights_list):
    ...
    def importances(model, X_valid, y_valid, features=None, n_samples=5000, sort=True, 
                    metric=None, sample_weights = None, higher_is_better = True, random_state = None):
        """
    ...
    rf1 = rf.fit(data1, y1)
    f1_score1 = metric_our(rf1, data2, y2, None)
    
    tm = datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")
    print ('First random forest end', '| time: ' , tm, 'score train=', 
           metric_our(rf1, data1, y1, None), 'score=', f1_score1)
    
    rf2 = rf.fit(data2, y2)
    f1_score2 = metric_our(rf2, data1, y1, None)
    
    tm = datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")
    print ('Second random forest end', '| time: ' , tm, 'score train=', 
           metric_our(rf2, data2, y2, None), 'score=', f1_score2)
        ...
        if several_perms == None:
            ...
            for i in range(several_perms):
                ...
                import_new1 = importances(rf1, data2, y2, metric=metric_our, n_samples = n_samples_our, 
                                         features = features_list, higher_is_better = higher_is_better, random_state = random_state+i+1)
                tm = datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")
                print ('First importance end', '| time: ' , tm)
...
def bootstrap(X_1, y_1, X_2,  y_2, pass_model, 
    ...
    for k in range(1, 1+nreps):
        ...
        if type(params_to_fit) != type(None):
            model = pass_model(**params_to_fit)
        elif type(params_to_fit) == type(None) and type(meta) != type(None) and type(by_var) != type(None) and params_dict != None:
        ...
        if hasattr(m, 'coef_') and hasattr(m, 'intercept_'): 
            betas = m.coef_.tolist()[0] + m.intercept_.tolist()
        ...
        if k == 1:
            scores1.to_csv(out + 'bootstrap_scores_train.csv', index=False)
            scores2.to_csv(out + 'bootstrap_scores_test.csv', index=False)
                
            train_scores = scores1
            test_scores = scores2
                
        else:
            with open(out + 'bootstrap_scores_train.csv', 'a') as outf:
                scores1.to_csv(outf, header=False, index=False)
            with open(out + 'bootstrap_scores_test.csv', 'a') as outf:
    ...
    if hasattr(m, 'coef_') and hasattr(m, 'intercept_'):
        betas_box1 = pd.DataFrame.from_records(betas_box, columns = list(X_tr.columns)+['Intercept'])
...
def br_stat(data, index_month, target, alpha = 0):
    """
...
def br_correction(data, index_month, target, correct = False, random_state = 42, alpha = 0):
    """
    Выравнивание BR 
    data - датасет
    index_month - переменная, по которой выравниваем BR 
    ...
    bad_rate_by_month = br_stat(data, index_month, target, alpha)
    
    if correct == False:
        plt.plot(bad_rate_by_month[index_month], bad_rate_by_month['bad_rate'], '.-') 	# Исправлено
        plt.plot(bad_rate_by_month[index_month], bad_rate_by_month['max_br_alpha'], '.-') 	# Исправлено
        return 
...
#         print(str(int(k)) + ' ' + str(k)+ ' ' + ' ' + str(X_tmp.shape))
    ...
    bad_rate_by_month = br_stat(data, index_month, target)    
    plt.plot(bad_rate_by_month[index_month], bad_rate_by_month['bad_rate'], '.-') # Исправлено
...
def linear_calib_use_model_data(train, test, oot, target, model,
    ...
    if preprocessed == False:
        ...
        X_1_2 = data_preprocessing_test(train, y_true_train, necessary_fields_upper, categorial_list = 
        ...
        X_2_2 = data_preprocessing_test(test, y_true_test, necessary_fields_upper, categorial_list = 
                                        list_of_categories_sel, 
                                        drop_technical = True,
                                    attribute_list = attribute_list_model, var_col = var_col,
                                median = mediana,
...
def for_splines(X_1, X_2, y_train, categorical_cols, max_d =2, random_state = 241, task = 'binary'):
    ...
    for i in columns:
        if i not in categorical_cols:
            ...
            while np.max(bins1) > max_all and len(bins1) > 1:
                print('max', len(bins1), np.max(bins1), max_all)
                bins1.pop(-1)
                zero_columns_right.append(columns_spline[-1])
                columns_spline.pop(-1)
            
            if np.max(bins1) <= max_all and np.min(bins1) >= min_all:
                array_bs_test1 = bs(train_test, degree = 0, knots = bins1, include_intercept = True)
                array_bs_test = pd.DataFrame(array_bs_test1, 
                                         columns = columns_spline[:array_bs_test1.shape[1]])
                X_2_spline = X_2_spline.join(array_bs_test)
                
            elif len(bins1) == 1 and np.min(bins1) < min_all:
                X_2_spline[columns_spline[0]] = 0
                X_2_spline[columns_spline[1]] = 1

            elif len(bins1) == 1 and np.max(bins1) > max_all:
                X_2_spline[columns_spline[0]] = 1
                X_2_spline[columns_spline[1]] = 0 
...
def find_meta_params_optuna(X, Y, params_dictionary, pass_model, 
                     sort_by_var, list_of_vars_for_strat, n_folds, second_target, yeo_johnson, 
                     attribute_list, var_col, categorial_list = None, cols_outlier = None, need_business = True, 
                     draw = True, draw_by_approval_rate = False,
                     simple_b_score = None, business_dict = None, business_dict_sec = None,
                     ...
                     high_outlier = None, 
                     low_outlier = None, check_percentile = 1, random_state = None, task = 'binary', k_logs = 10,
                     cut_non_out_9999 = True, optimize_metric="AUC",
                     direction='maximize', n_trials=1, path="", n_jobs=2,a=0.01, k_fold_method="mean",additional_find=False):
...
def get_stats_on_target(data, columns, time, target, model_name, technical_values=None, category_list = None, 
                       group = True, n_jobs=1, verbose=20,
                       category_target = True,pvalue = 0.05, prep_dict=None, period_verbose=False):
    
    """
    Посчитать стабильности переменных по периодам.
    Используются проверки:
        Метрика < 0.05
        Знакопеременная метрика
        Знакопеременность betas
        Значимость связи переменной и таргета
    data - данные
    columns - переменные
    time - переменная, которая отвечает за время
            ...
            for i in range(0,len(unique),n_periods_in_group):
                if (i == time_len) or (((n_periods_in_group-(len(unique)%n_periods_in_group))>1) and (len(unique)%n_periods_in_group)!=0):
                    ...
                    if(category_target):
                        ...
                        metric_month=r2_score(y, y_pred)
                    ...
                    if(metric<0.05):
                        stabilty="Unstable"  
                        stable_gini="+"
                    if(metric>0 and metric_month<0):
                        stabilty="Unstable"  
                        stable_gini_period="+"
                    #########################################################
    ...
    if type(prep_dict) is not type(None):
        ...
        data=data_preprocessing_test(data, target, technical_values=technical_values,
                                     categorial_list = category_list, drop_technical = False,
                                    **prep_dict)
    else:
        data=data_preprocessing_train(data, target, technical_values=technical_values,
                                     categorial_list = category_list, drop_technical = False,
                                    yeo_johnson = False, attribute_list = None, var_col = None,
...
def get_stats_on_target_by_month(data, columns, time, target, model_name,
                                 ...
                                 n_jobs=1, verbose=20, category_target = True,
                                 pvalue = 0.05, prep_dict=None):
    """
...
#     for o in range(len(columns)):
        ...
        for n_periods_in_group in range(1,len(unique)):
            ...
            if(category_target):
                ...
                y_pred=model_month.predict(x_label)
                Gini_train=r2_score(y, y_pred)
                month_coef = model_month.coef_[0]
                y_val_pred=model_month.predict_proba(x_val_label)
                Gini_valid=r2_score(y_val, y_val_pred)
...
def feature_importance(X_1, y_1, X_2, y_2, w_b, task="binary", boosting=False, treshold=0.005, n_jobs=1, selection="SHAP", selection_politic="Hard"):
    ...
    - selection - Boruta, SHAP или Boruta_SHAP
    ...
    def tree_study(X_1,y_1,X_2,y_2,w_b,task,boosting,treshold):
        ...
        if selection=="Boruta":
            ...
            X = X_1[features].values
            Y = y_1.values.ravel()
            boruta_feature_selector = BorutaPy(rf, n_estimators='auto', verbose=2, random_state=4242, max_iter = 50, perc = 100, pvalue=0.02)
            ...
            final_features_global.append(final_features)
        elif selection=="Boruta_SHAP":
...
#             print(Feature_Selector.Subset().columns
            ...
            shap_values.append(explainer.shap_values(X_2[imp_test]))
            ...
            if task == 'binary':
                ...
                y_pred_test = rf.predict(X_2[imp_test])
                auc.append(r2_score(y_2, y_pred_test))
    ...
    print("Start first tree", datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S"))
    from joblib import Parallel, delayed
    ...
    if((selection=="Boruta") | (selection=="Boruta_SHAP")):
        ...
        metric2=metric1
        print("Finish first tree", datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S"))
        while(metric1-metric2<treshold*metric1):
            ...
            if ((importance_df.iloc[-1]["shap_importance"] != 0) | (step>1)):
            ...
            imp_test=importance_df["column_name"].to_list()
